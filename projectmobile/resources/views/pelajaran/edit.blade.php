...
<form method="post" action="/pelajaran/{{$pelajarans->pelajaran_id}}">
    @method('PUT')
    @csrf
    ...
    <div class="mb-2">
      ...
      <select class="form-select @error('nip_guru') is-invalid @enderror" name="nip_guru">
        @foreach($gurus as $guru)
          @if(old('nip_guru', $pelajarans->nip_guru)== $guru->nip_guru)
    ...
    <div class="mb-2">
      ...
      <input type="text" class="form-control @error('nama_pelajaran') is-invalid @enderror" name="nama_pelajaran" value="{{ old('nama_pelajaran', $pelajarans->nama_pelajaran) }}">
